...
function toObservable2(source, options) {
  if (ngDevMode && !options?.injector) {
    assertInInjectionContext(toObservable2);
  }
...
function routeChangeSignal(router, compareStrictly = true) {
  const routes = signal(["", ""], ...ngDevMode ? [{ debugName: "routes" }] : []);
  router.events.pipe(filter((event) => event instanceof NavigationEnd), map((event) => {
    const url = event.url;
    return compareStrictly ? url : url.split("?")[0].split("#")[0];
  }), pairwise()).subscribe((urls) => {
    if (urls[0] !== urls[1]) {
      routes.set(urls);
    }
  });
...
@angular/core/fesm2022/rxjs-interop.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-4GKKNB6P.js.map
