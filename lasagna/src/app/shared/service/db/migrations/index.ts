...
export const migrations: {
  ...
  // Добавление expirationDate (срок годности) для продуктов
  {
    version: 10,
    schema: {
      [Stores.PRODUCTS]: '++uuid,name,source,brand,cloud_uuid,deleted',
      ...
      return new Promise(async (resolve) => {
        ...
        await Promise.all(products.map((product: any) => {
          if (product.expirationDate == null) {
            return productsTable.update(product.uuid, {expirationDate: 0});
          }
          return Promise.resolve();
        }));
  ...
  // Добавление cleaningLoss (отход при очистке) для продуктов
  {
    version: 11,
    schema: {
      [Stores.PRODUCTS]: '++uuid,name,source,brand,cloud_uuid,deleted',
    ...
    update: tx => {
      return new Promise(async (resolve) => {
        ...
        await Promise.all(products.map((product: any) => {
          if (product.cleaningLoss == null) {
            return productsTable.update(product.uuid, {cleaningLoss: 0});
          }
          return Promise.resolve();
        }));
