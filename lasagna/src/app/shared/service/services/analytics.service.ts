...
export class AnalyticsService {
  ...
  private _getUserUUID(): string {
    try {
      const autoGenerated = String(this._window?.localStorage.getItem('userUUID')).trim();
      const devReplaced = JSON.parse(this._window?.localStorage.getItem('dev_settings') || '{}');
      return String(
        devReplaced['userUUIDStr'] || autoGenerated
      ).trim() || 'unknown';
    } catch {
      return 'unknown';
    }
