...
export const productLabelFactory = (product?: ProductDTO, t?: (s: string) => string) => {
  if (!product) {
    return 'unknown';
  }
...
export const productLabelFactoryProvider = new InjectionToken('productLabelFactory', {
  factory: () => {
    ...
    return (product?: ProductDTO) => productLabelFactory(product, (s: string) => translateService.instant(s));
  }
});
