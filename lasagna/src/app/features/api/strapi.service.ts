...
export class StrapiService
  ...
  get<T = any | any[]>(url: string, params?: Record<string, any>) {
    return this.restService.get<StrapiResponse<T>>(this.endpoint + url, params);
  }

  post<T>(url: string, data?: Record<string, any>) {
    return this.restService.post<StrapiResponse<T>>(this.endpoint + url, {data});
  }

  put<T>(url: string, data?: Record<string, any>) {
    return this.restService.put<StrapiResponse<T>>(this.endpoint + url, {data});
  }
