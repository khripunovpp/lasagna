...
@Component({
  selector: 'lg-system-credential-settings',
  standalone: true,
  template: `
      <lg-gap-column>
...
export class SystemCredentialSettingsComponent {
  constructor(
  ...
  ngOnInit() {
    this._credentialsRepository.getAllSystemCredentials().then((credentials) => {
  ...
  addRow() {
    ...
    this.credentials.update(current => [...current, new Credential({type: CredentialsType.system})]);
    this.form.markAsDirty();
  }
